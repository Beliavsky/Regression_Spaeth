...
!
!    Input, integer ( kind = 4 ) IOUT, the index of V1 that is not to 
!    be changed.
!
  implicit none
...
!
!    Input/output, integer ( kind = 4 ) IBASE(N), a list of rows in the order 
!    they should be used.  If INTL is TRUE, then IBASE is an output quantity
...
!
!    Output, real ( kind = 4 ) SN, SZ, SP, the sum of the absolute values
!    of U(I) for which U(I) * V(I) is negative, zero, or positive respectively.
!
  implicit none
...
!
!    Input, integer ( kind = 4 ) CODE, indicates additional sign constraints
!    that may be placed on the solution X, and on the sign of A*X-B.
...
!    to take.
!
!    Input/output, real ( kind = 4 ) X(N).  Used for input if CODE = 1.  On 
...
!*****************************************************************************80
!
!! CON_LI minimizes L-infinity norm of A * X - B subject to linear constraints.
!
!  Discussion:
!
!    The constraints have the form:
...
!    Input, real ( kind = 4 ) E(K,N), the inequality constraint matrix.
!
!    Input, real ( kind = 4 ) F(K), the lower bounds for the 
!    linear inequalities.
!
!    Input, real ( kind = 4 ) G(K), the upper bounds for the 
!    linear inequalities.
!
...
!    an improved partition.
!
!    Input/output, integer ( kind = 4 ) ML, MU, the minimum and maximum number
!    of observations to be allowed per cluster.  ML must be greater than N.
!
!    Output, real ( kind = 4 ) X(S,N), contains the computed parameter vectors.
!
!    Output, real ( kind = 4 ) E(S), the sum of squared residuals.
...
!
!    Input, real ( kind = 4 ) A(M,N), B(M), the coefficient and right hand side
!    data.
!
...
!
!    Input, real ( kind = 4 ) EPS2, a tolerance for convergence of 
!    the iteration.
!
!    Input, integer ( kind = 4 ) ITMAX, the maximum number of iterations.
!
...
!    Output, real ( kind = 4 ) X(N), the solution vector.
!
!    Input, real ( kind = 4 ) LAMBDA, the value of LAMBDA in the objective 
!    function.
!
!    Output, integer ( kind = 4 ) IFLAG, error flag.
...
!  Parameters:
!
!    Input, real ( kind = 4 ) A(M,N), the matrix from which submatrices 
!    are selected.
!
!    Input, integer ( kind = 4 ) M, the number of rows of A.
!
...
!
!    Output, integer ( kind = 4 ) NA, the number of columns that were selected
!    to construct the current submatrix.
!
...
!    selected variables.
!
!    Output, integer ( kind = 4 ) KBIT(N), contains a 1 for those variables 
!    which are included, and a 0 for those which are excluded.
!
...
!
!    Output, real ( kind = 4 ) U(M,N), contains the matrix U, with orthogonal 
...
!    decomposition is desired, and to FALSE otherwise.
!
!    Output, real ( kind = 4 ) V(N,N), the orthogonal matrix V of the 
...
!    Input, real ( kind = 4 ) B(M), the right hand side vector.
!
!    Input, real ( kind = 4 ) EPS, used for an accuracy test.  Singular values 
!    less than or equal to EPS will be regarded as zero.
!
!    Output, real ( kind = 4 ) X(N), the least squares minimizing solution.
!
!    Output, real ( kind = 4 ) COND, the ratio of the largest to smallest 
!    singular value if the matrix A has maximal rank N, or HUGE(COND) if one
...
!    at least M.
!
!    Input, integer ( kind = 4 ) NB, the number of right hand sides; that is, 
!    the number of columns of data in B.  If no right hand sides are being
!    supplied, set NB = 0.
!
...
!  Parameters:
!
!    Input/output, integer ( kind = 4 ) ISEED, the integer "seed" used to 
!    generate the output random number, and updated in preparation for the
!    next one.  ISEED should not be zero.
!
!    Output, real ( kind = 4 ) UNIFORM_01_SAMPLE, a random value between 0 
!    and 1.
!
!  Local Parameters:
!
...
!  Parameters:
!
!    Output, real ( kind = 4 ) URAND, a uniformly distributed pseudo 
!    random number.
!
  implicit none
